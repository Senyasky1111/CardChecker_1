...
class TileDataset(Dataset):
    def __init__(self, root: Path, split: str, tile=512, stride=4, train=True):
        self.items = []  # (img_path, [(tx,ty), ...]); empty list = negative
    ...
    def _gauss(self, pts):
        """render gaussian at each (cx,cy) heatmap-coord point, max-combined."""
    ...
    def __getitem__(self, i):
        ...
        hm = self._gauss([(tx * self.hm, ty * self.hm) for tx, ty in pts])
        return x, torch.from_numpy(hm), (1 if pts else 0)
...
def evaluate(model, loader, device, thr=0.3, tol=6):
    ...
    for x, hm_gt, ispos in loader:
        ...
        for b in range(x.size(0)):
            pk = peaks(hm[b, 0], thr)
            if ispos[b].item() == 1:
                gpts = peaks(hm_gt[b, 0], thr=0.5)  # recover all GT points (gaussian peaks)
                matched_pred = set()
                hits = 0
                for (gy, gx) in gpts:
                    for j, (py, px) in enumerate(pk):
                        if j not in matched_pred and abs(py - gy) <= tol and abs(px - gx) <= tol:
                            matched_pred.add(j); hits += 1; break
                tp += hits; fn += max(0, len(gpts) - hits)
                fp += max(0, len(pk) - hits)  # pred peaks matching no GT point
            else:
...
def main():
    ...
    npos = sum(1 for _, pts in tr.items if pts)
