"""Tar + split + upload data/tag_v5_tiles to HF as v5_parts/part_NNN.
Pure Python (consistent paths) — avoids the bash /tmp vs Windows-python mismatch that
made the first upload glob 0 files. Matches bootstrap_exp1.sh which does `cat v5_parts/part_*`.
"""
import os, tarfile, glob, math
from pathlib import Path

REPO = Path("d:/CardChecker")
SRC = REPO / "data/tag_v5_tiles"
WORK = REPO / "data/_v5up"
PART_MB = 480
...
tar_path = WORK / "v5.tar.gz"

print("[tar] packing v5", SRC, flush=True)
with tarfile.open(tar_path, "w:gz") as tf:
    tf.add(SRC, arcname="tag_v5_tiles")
...
for pp in parts:
    api.upload_file(path_or_fileobj=str(pp), path_in_repo=f"v5_parts/{pp.name}",
                    repo_id=REPO_ID, repo_type="dataset", commit_message=f"v4 {pp.name}")
    print("[upload]", pp.name, flush=True)
print(f"[done] uploaded {len(parts)} parts to HF v5_parts/")
