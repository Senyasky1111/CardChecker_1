...
cat /workspace/v4_parts/part_* > /workspace/_v4.tar.gz
mkdir -p /workspace/data
tar --no-same-owner -xzf /workspace/_v4.tar.gz -C /workspace/data
rm -rf /workspace/v4_parts /workspace/_v4.tar.gz
echo "tiles:"; for s in train val test; do echo -n "  $s: "; ls /workspace/data/tag_v4_tiles/images/$s 2>/dev/null | wc -l; done
...
python3 scripts/train_defect_exp1.py --data /workspace/data/tag_v4_tiles \
  --sample 80 --overfit --epochs 30 --batch 16 --out /workspace/runs/exp1_sanity 2>&1 | tee /workspace/sanity.log | tail -34
echo "=== SANITY DONE ==="
...
for f in ['/workspace/sanity.log','/workspace/runs/exp1_sanity/log.json']:
    if os.path.exists(f):
        api.upload_file(path_or_fileobj=f, path_in_repo=f'results/exp1_{os.path.basename(f)}',
...
SF1=$(python3 -c "import json;d=json.load(open('/workspace/runs/exp1_sanity/log.json'));print(max(r['f1'] for r in d))" 2>/dev/null || echo 0)
...
if python3 -c "import sys;sys.exit(0 if float('$SF1')>0.5 else 1)"; then
  ...
  python3 scripts/train_defect_exp1.py --data /workspace/data/tag_v4_tiles \
    --epochs 25 --batch 48 --out /workspace/runs/exp1_full 2>&1 | tee /workspace/full.log | tail -40
  python3 - <<'PY'
import os
from huggingface_hub import HfApi, login
login(token=os.environ['HF_TOKEN']); api=HfApi()
for f in ['/workspace/full.log','/workspace/runs/exp1_full/log.json','/workspace/runs/exp1_full/best.pt']:
    if os.path.exists(f):
        api.upload_file(path_or_fileobj=f, path_in_repo=f'results/exp1_{os.path.basename(f)}',
