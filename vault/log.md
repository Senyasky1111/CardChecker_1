...
- 2026-06-22 — Pregrading GRADING-LOGIC overhaul on live stakeholder feedback (3-agent design discussion: CV / grading-domain / product-UX, all grounded in vault). Decisions: (1) **Aggregation = weakest-link, NOT average** (one bad subgrade caps; `[10,10,10,4]→5`) per PSA/BGS docs. (2) **TAG calibration `1.58·raw−4.88` DROPPED for display** — it dragged sides 8/7.5 → headline 7, contradicting the visible sub-grades; headline = model's weighted overall; `calibrate()` kept for a future PSA recalibration. (3) **Centering subgrade from geometry**, not the grader. (4) **Graded whitening cap** replaces the blunt 6-zone floor: 1 MOD+→max9, 2→7, 4→5, HEAVY harsher; one-way; and a back with CONFIRMED MOD+ whitening loses its front-primary `BACK_LENIENCY` (this was the Vaporeon whitened-back miss). (5) **Gate = always inspect (detector is free/concurrent); cap fires on detector–holistic DISAGREEMENT**, not "skip confident 9-10" (the grader's own confidence is unreliable → would skip the silently-overrated cards). (6) **Surface stays in the holistic grade but is NOT zone-localized** (can't detect from one flat photo; but it's the strongest pillar so don't drop it). (7) **Corner/edge crops + montage + detector now cut from the USER-confirmed centering geometry** (rectified card + dragged outer box, both sides) instead of bg-segmentation `card_box` — precise + fixes phone-photo crop break + detector recall on real edges; `/grade` takes `front/back_warped_url`+`front/back_outer`, path-traversal-safe, falls back to card_box. (8) Report shows 8 corners + 8 edges crops with severity badges (collapsible). (9) No glare-check (stakeholder accepted the false-positive-on-glare risk). (10) OverloadedError(529)→503 + 60s client timeout (was surfacing as "timed out"). 77+ backend tests green; rebuilds #5–#8 all deployed + healthy; frontend all merged to main. NEXT: stakeholder retest on real cards → tune detector recall if it still under-flags. See [[10-Projects/2026-Q2-pregrading-integration]] (methodology-superseded callout) + memory [[project_pregrading_integration]].
