...
## ⚠️ GRADING METHODOLOGY SUPERSEDED 2026-06-22 (stakeholder, on first live cards)
The B1/B1b calibration + weighted-average below are **no longer how the grade is computed.** After live feedback ("grading isn't a weighted average; one bad subgrade caps the card") and confirming published PSA/BGS rules, the aggregation is now:
- **Weakest-link, not average** (`pd.weakest_link`): overall = the lowest subgrade, +≤1 bump only if the others are much higher. One bad attribute caps the card (`[10,10,10,4]→5`). PSA: overall ≈ worst attribute; BGS: overall never >1 above the lowest subgrade.
- **Per side** = weakest-link of its 4 subgrades; **overall** = weakest side, but **FRONT-primary** (back gets ~1 grade of leniency — `BACK_LENIENCY` in `pregrade_service.py` — a back flaw drags less than the same flaw on the front).
- **Centering subgrade = geometry measurement** (worst-axis offset → PSA table `pd.centering_grade_from_offset`), NOT the grader's eyeball. `/grade` takes `front/back_centering_off` (the webapp sends the `worst_axis_offset_pct` from the centering step).
- **TAG calibration `1.58·raw−4.88` DROPPED for display** — it dragged the headline below the visible sub-grades (sides 8/7.5 → 7), contradicting them; TAG is stricter than PSA mid-range and we show a PSA-style scale. `calibrate()` is retained in code for a FUTURE recalibration against a **PSA-graded** reference set (the real accuracy fix — get 20-30 PSA cards).
- **Display σ tightened** (medium 1.76→1.0, `SIGMA`) so the most-likely grade reads clearly; the TAG-residual width no longer applies. This σ is now a plain ±1 estimate band, not a TAG-validated coverage figure.
The integer-binning (B1b) + safety-floor (B2) + detector-evidence + gate all still apply. The headline grade = `most_likely` integer mode of the distribution centered on the weakest-link overall.
...
> **Integer-binning fix 2026-06-22 (live-test bug).** First live Decision Card showed bars summing to ~52% (10:11% 9:30% 8:11%). Cause: `build_overall` returned the distribution on the **half-grade grid** (8.5, 9.5, …) but the frontend renders **integer PSA rows only**, so the half-grade mass (~48%) was silently dropped. Fix (`src/pregrade_distribution.integer_distribution()`): fold half-grade mass into integers (.5 split 50/50 to floor/ceil, clamped [1,10]), renormalize, round so it **sums to exactly 100%**; `build_overall.most_likely` is now the **integer mode** (matches the tallest bar); `bucket`/`label` still from the continuous calibrated Ĝ (thresholds unchanged). Example Ĝ≈9.0 now renders **9:53% 10:22% 8:23% 7:2%**. The half-grade `grade_distribution()` is kept intact for internal use. ⚠️ **Caveat:** after integer binning the `score_empirical_coverage.py` "integer-bin coverage" reads **100%** — but that's near-trivial, because medium cards now render up to ~9 integer bars (incl. 1–2% tails), so almost any integer TAG lands in *some* bar. The meaningful accuracy signal is **within ±1 of Ĝ = 65%** (unchanged; bar-count-independent). **Open UI item:** consider hiding/grouping <~3% tail bars on the Decision Card for readability (must NOT drop their mass from the sum) — and re-measure coverage on the *visible* bars if tails are hidden.
